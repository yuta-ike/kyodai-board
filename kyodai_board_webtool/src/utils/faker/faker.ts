...
const getRandMul = (arr: any[]) => arr.filter(() => Math.random() > 0.7)

const getRandInt = (min: number, max: number) => Math.floor(Math.random() * (max - min)) + min;

const getRandFloat = (min = 0, max = 1) => Math.random() * (max - min) + min

export { getRand, getRandMul, getRandInt, getRandFloat, getRandomClubName, getRandomNickName, getRandomSportsName, getRandomImage, getRandomIcon };